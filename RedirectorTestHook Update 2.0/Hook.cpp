...
std::vector<DWORD> MemoryAddr(Files.size());

std::vector<DWORD> AllocatedFileAddr(Files.size());
		...
		for (auto& file : Files) {
			try {
				...
				{		
						...
						if (file.extension == "tex" && std::stoul(file.component) == 0) {
							PatchTexture(file.name, file.data);
							
							std::string outName = "patched_" + file.name;
							DumpFile("RedirectorTest\\" + outName, file);
						}
						
						
						if(file.extension == "mesh" && std::stoul(file.component) == 0) {
							ParseAndPatchMesh(file.data);
						...
						//char* AllocatedFilename = allocateFilename(file.name);
						...
						AllocatedFileAddr[m] = (DWORD)file.data;
