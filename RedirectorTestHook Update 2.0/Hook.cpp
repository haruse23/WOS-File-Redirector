...
DWORD InitiateHookPointer() {
	
	DWORD pointerPatch = FindPattern("Spider-Man Web of Shadows.exe", "\x8B\x03\x83\x38\xFF\x75\xB0\x5F\x5E\x5D\x83\x03\x04\x5B\xC2\x08\x00", "xxxxxxxxxxxxxxxxx");
	
	//MsgBoxOmar(pointerPatch);
	
	pointerPatchJmpBack = (BYTE*)pointerPatch + 0x5;
	
	//MsgBoxOmar((DWORD)pointerPatchJmpBack);
	
	PlaceJMP((BYTE*) pointerPatch, (DWORD)RedirectReferences, 0x5);
...
char* allocateFilename(const std::string& filename)
{
...
void DumpFile(const std::string& outPath, const LoadedFile& file) {
...
DWORD FileCount = Files.size();

std::vector<DWORD> MemoryAddr(FileCount);

std::vector<DWORD> AllocatedFileAddr(FileCount);
	...
	if(FilenameHash) {
		...
		for (int m = 0; m < FileCount; m++) {
			try {
				...
				if(std::stoul(Files[m].hash, nullptr, 0) == FilenameHash && std::stoul(Files[m].component) == FileComponentRegister) 
				{		
						
						if (Files[m].extension == "tex" && std::stoul(Files[m].component) == 0) {
							PatchTexture(Files[m].name, Files[m].data);
							
							std::string outName = "patched_" + Files[m].name;
							DumpFile("RedirectorTest\\" + outName, Files[m]);
						}
						
						
						if(Files[m].extension == "mesh" && std::stoul(Files[m].component) == 0) {
							ParseAndPatchMesh(Files[m].data);
						...
						char* AllocatedFilename = allocateFilename(Files[m].name);
						
						*(DWORD*)(FileHeaderRegister) = (DWORD)AllocatedFilename;
						...
						MemoryAddr[m] = *(DWORD*)(FileHeaderRegister + FileComponentRegister * 4 + 8); // BeforeAllocation
						
						
						
						AllocatedFileAddr[m] = (DWORD)Files[m].data;
						
						*(DWORD*)(FileHeaderRegister + FileComponentRegister * 4 + 8) = AllocatedFileAddr[m];
				...
				// file.hash or file.component was not a valid number
				// skip this entry
				continue;
				}
...
DWORD WINAPI FixReferences(){
	...
	for(int m = 0; m < FileCount; m++){
		if(PatcherValue == MemoryAddr[m] || PatcherValue2 == MemoryAddr[m]) {
			*TargetPtrAddr = AllocatedFileAddr[m];
			
			//char buf[64];
			//sprintf(buf, "AllocAddress: %u", AllocatedFileAddr[m]);
			//MessageBox(0, buf, "DEBUG", MB_OK);
			
			//sprintf(buf, "MemoryAddr: %u", MemoryAddr[m]);
			//MessageBox(0, buf, "DEBUG", MB_OK);
	...
	LPVOID lpReserved)
{
	switch(fdwReason)
	{
		...
		case DLL_PROCESS_ATTACH:
		{
			...
			//void* addr = (void*)&FixReferences;

			
			//char buf[128];
			//sprintf_s(buf, "FixReferences address: 0x%p", addr);
